...
> **What this is**: the answers to the plan's §2 questions
> ([`plans/implementation-plan.md`](plans/implementation-plan.md)), each one
> checked against the running deployment rather than against the documentation.
...
The plan warns about this and the warning was correct. Confirmed against the
live services:
...
| | |
|---|---|
| **Status** | ✅ confirmed — **better than the plan assumed** |
...
| | |
|---|---|
| **Status** | ✅ **available on the OIDC surface** — the plan said "don't assume this" and the assumption would have been safe |
...
| **Documented risk** | the guide notes *"a new World identity can resolve to a new IdP account"*. So continuity is stable for an existing identity but is not a promise about a re-enrolled one. Recorded as a limitation in [`INTEGRATION_DEBRIEF.md`](INTEGRATION_DEBRIEF.md). |
| **Fallback** | not needed. |
...
| **What the plan assumed** | "the exact shape of the verification endpoint" — a World-ID-style endpoint you POST a proof to and receive a nullifier from. |
...
in [`INTEGRATION_DEBRIEF.md`](INTEGRATION_DEBRIEF.md).
...
| **Fallback** | this is the shipped design. Had the plan's assumption held, the upgrade path would be to add RFC 9728 metadata at `/.well-known/oauth-protected-resource` on our own server and validate IdP-issued tokens — `worldid/` is the only module that would change. |
...
Recorded because the plan asks for them, and because they are the raw material
for [`INTEGRATION_DEBRIEF.md`](INTEGRATION_DEBRIEF.md).

1. **The plan's model of the verification surface was wrong.** It assumes a
...
2. **`frontend / backend` split assumed by the plan's `action` binding does not
   map cleanly.** The plan says `action` must be bound to the purchase so the
