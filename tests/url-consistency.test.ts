...
test('the registered redirect URI decides the origin when nothing else does', () => {
  ...
  withEnv(
    { HUMANGATE_PUBLIC_URL: undefined, WORLDID_REDIRECT_URI: 'https://localhost:3000/api/auth/world/callback' },
    () => {
...
test('a tunnel hostname propagates to every derived URL', () => {
  withEnv(
    { HUMANGATE_PUBLIC_URL: undefined, WORLDID_REDIRECT_URI: 'https://abc-def.trycloudflare.com/api/auth/world/callback' },
    () => {
...
test('an explicit public base URL is honoured', () => {
  withEnv({ HUMANGATE_PUBLIC_URL: 'https://humangate.example', WORLDID_REDIRECT_URI: undefined }, () => {
    assert.equal(publicBaseUrl(), 'https://humangate.example');
    assert.equal(redirectUri(), 'https://humangate.example/api/auth/world/callback');
  });
});
...
test('the two settings agreeing on origin is consistent, even with different paths', () => {
  withEnv(
    {
      HUMANGATE_PUBLIC_URL: 'https://localhost:3000',
...
test('a scheme disagreement is reported, and names both sides', () => {
  ...
  withEnv(
    {
      HUMANGATE_PUBLIC_URL: 'http://localhost:3000',
      WORLDID_REDIRECT_URI: 'https://localhost:3000/api/auth/world/callback',
    },
    () => {
...
test('a malformed redirect URI is reported rather than thrown at link-building time', () => {
  withEnv({ HUMANGATE_PUBLIC_URL: undefined, WORLDID_REDIRECT_URI: 'not a url' }, () => {
