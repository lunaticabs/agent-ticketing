 ...
 * `HUMANGATE_DB` and caches a connection on `globalThis`.
 */
...
process.env.HUMANGATE_DB = path.join(dir, `test-${process.pid}-${Date.now()}.db`);
process.env.HUMANGATE_SIGNING_KEY =
  process.env.HUMANGATE_SIGNING_KEY ?? 'humangate-test-signing-key-0123456789abcdefghij';
...
process.env.HUMANGATE_PUBLIC_URL = 'http://localhost:3000';

export const TEST_DB_PATH = process.env.HUMANGATE_DB;
