 ...
 *
 * Split out of `page.tsx` for two reasons: Next only permits a fixed set of
 * exports from a page module, and reading `useParams` needs a mounted router,
 * which a test environment does not have. The routing shell is the two-line
 * `page.tsx` next door; everything worth exercising lives here.
 */
...
/** The transfer as the API returns it. */
interface TransferViewData {
  ok: true;
  transferId: string;
  slotId: string;
  from: string;
...
/**
 * The recipient's screen, with the token supplied as a prop.
 */
export function TransferView({ token }: { token: string }) {


  const [view, setView] = useState<TransferViewData | null>(null);
  ...
  const load = useCallback(async () => {
    try {
      setView(await call<TransferViewData>(`/api/transfer/${token}`));
    } catch (err) {
