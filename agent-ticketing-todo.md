...
> **实现时的范围收缩（2026-09-25）**：本项目最终**只做 `locked` 一种模式**——名额
> 锁定在中签者身上，不可转让。转让引擎（M4 全部）、三级策略旋钮、按 continuity 的
> 入账上限，以及演示第 4/5 拍中的转让部分都已移除，本文档相应章节同步删除。
> 保留下来的红线是 1–6 和 10；红线 7/8/9 随转让一起作废（它们描述的都是接收方的
> 窗口与上限）。这削弱了"持有与流转层"的叙事，换来的是最强的防黄牛姿态：名额根本
> 不离开中签的人。
...
一个活动排队系统：用户的 agent 替他排队，名额到手时要求真人做一次**新鲜验证**；人不在，名额**顺延**给下一个真人背书的 agent。名额锁定在中签者身上，不可转让。
...
| T-2.x 队列 / 抽签 / 顺延 | ❌ **完全没有** | ✅ **我们的核心增量** |
| 审批消费存储 | `signal` 绑定 + nullifier 一次性 = 原语 | ✅ **消费存储必须自己写**——官方 HITL 示例里 `consumeApproval` 是 `declare function`（只有声明，没有实现） |
...
> 官方提供的是**原语和协议**，不提供**产品**。
> 队列、抽签、顺延、审计留痕、demo 道具——**这几样没有任何官方替代品，全是我们的增量**；
...
| ~~7~~ | ~~顺延/接收窗口的 TTL 从「接收方打开」起算~~ | **随转让作废** |
| ~~8~~ | ~~转让必须由接收方本人完成新鲜验证~~ | **随转让作废** |
| ~~9~~ | ~~入账上限按 continuity 标识计~~ | **随转让作废**——但按人计数的原则保留在队列唯一性上（同一连续性的第二个账号买不到第二个位置） |
...
- [ ] 拍 3：人不批准 → 倒计时归零 → **名额顺延**
- [ ] 拍 4：**40 个账号加入队列 → 塌缩成 2 个 continuity 标识 → 只有 2 个排队位**
- [ ] 拍 5：回放 / 改参数 / 换环境 → 三连拒
...
原 M4「转让三防线」随转让引擎移除。名额锁定在中签者身上，没有 T-4.1–T-4.5。
